...
def getlist(processo, token):
    ...
    resposta = requests.post('https://api.projurisadv.com.br/adv-service/tarefa/consulta-sem-paginacao', headers=headers, json=proc_json)
    if resposta.status_code == 204:
        return False
    elif resposta.status_code == 200:
        resposta_json = resposta.json()
        return resposta_json["tarefasAgrupadasDataTipoWs"]
    else:
